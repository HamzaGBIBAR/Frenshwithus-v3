...
export default function AvailabilityGrid({
  ...
  const getSlotAt = useCallback((dayOfWeek, hour) => {
    return slots.find((s) => {
      ...
      // Slot crosses midnight (e.g. 19:00–00:00): endH=0 < startH
      const crossesMidnight = endH != null && endH <= startH;
      const endHour = endH ?? startH + 1;
      if (crossesMidnight) {
        return hour >= startH || hour < endHour; // 19–23 or 0–5 (we show 6–23, so hour >= startH)
      }
