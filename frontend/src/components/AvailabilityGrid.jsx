...
export default function AvailabilityGrid({
  ...
  const handleSlotClick = useCallback((e, slot) => {
    e.stopPropagation();
    if (onRemove && slot?.id) onRemove(slot.id);
  }, [onRemove]);
                      ...
                      >
                        {slot && isFirstOfSlot && (
                          <button
                            ...
                            className="absolute inset-0 w-full flex items-center justify-center text-[10px] sm:text-xs font-medium rounded-md hover:bg-red-500/20 hover:bg-opacity-80 transition-all duration-200"
                            title={removeLabel}
                          >
                            ...
                            {hoverSlot === slot.id && (
                              <span className="absolute right-1 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-red-500/90 flex items-center justify-center text-white text-[10px] font-bold animate-fade-in">
                                ×
                              </span>
                            )}
                          </button>
