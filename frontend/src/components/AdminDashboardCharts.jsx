...
const axisStyle = () => ({
  tick: { fill: isDark() ? 'rgba(245,245,245,0.7)' : 'rgba(26,26,26,0.8)', fontSize: 11 },
...
/** Merge trend arrays by month key — safe even when arrays have different lengths */
function buildChartData(subscriptionTrends, revenueByMonth, lessonsByMonth) {
  const monthMap = {};
  const ensure = (month) => {
    if (!monthMap[month]) monthMap[month] = { month, new: 0, cancelled: 0, active: 0, revenue: 0, lessons: 0 };
  };
  (subscriptionTrends.newStudentsByMonth || []).forEach(({ month, count }) => { ensure(month); monthMap[month].new = count ?? 0; });
  (subscriptionTrends.cancellationsByMonth || []).forEach(({ month, count }) => { ensure(month); monthMap[month].cancelled = count ?? 0; });
  (subscriptionTrends.activeByMonth || []).forEach(({ month, count }) => { ensure(month); monthMap[month].active = count ?? 0; });
  (revenueByMonth || []).forEach(({ month, total }) => { ensure(month); monthMap[month].revenue = total ?? 0; });
  (lessonsByMonth || []).forEach(({ month, count }) => { ensure(month); monthMap[month].lessons = count ?? 0; });

  return Object.values(monthMap)
    .sort((a, b) => a.month.localeCompare(b.month))
    .map((m) => ({ ...m, monthLabel: formatMonth(m.month) }));
}

const EmptyChart = ({ message }) => (
  <div className="h-full flex flex-col items-center justify-center gap-2 text-text/40 dark:text-[#f5f5f5]/30">
...
export default function AdminDashboardCharts() {
    ...
    return (
      <div className="rounded-2xl border border-red-200 dark:border-red-500/40 bg-red-50 dark:bg-red-900/20 p-6 text-center">
        ...
        <button
          type="button"
          onClick={fetchData}
          className="mt-4 px-4 py-2 rounded-xl bg-pink-primary dark:bg-pink-400 text-white font-medium hover:opacity-90 transition"
        >
          {t('dashboard.admin.retry', 'Réessayer')}
        </button>
      </div>
    );
  }
  ...
  const ChartCard = ({ title, subtitle, children }) => (
    <div className="group bg-white dark:bg-[#1a1a1a] rounded-2xl border border-pink-soft/40 dark:border-white/10 shadow-pink-soft dark:shadow-lg p-5 transition-all duration-300 hover:shadow-[0_12px_32px_rgba(231,84,128,0.15)] dark:hover:shadow-[0_12px_32px_rgba(231,84,128,0.1)] hover:border-pink-primary/20 dark:hover:border-pink-400/20 animate-fade-in overflow-hidden">
      ...
      <div className="min-h-[260px] min-w-[280px] w-full" style={{ height: CHART_HEIGHT }}>{children}</div>
    </div>
  );
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          <KpiCard label={t('dashboard.adminStats.totalStudents')} value={kpis.totalStudents ?? 0} icon={KpiIcons.students} delay={0} />
          <KpiCard label={t('dashboard.admin.totalProfessors')} value={kpis.totalProfessors ?? 0} icon={KpiIcons.professors} delay={50} />
          <KpiCard label={t('dashboard.admin.totalRevenue')} value={`€${Number(kpis.totalRevenue ?? 0).toFixed(0)}`} icon={KpiIcons.revenue} delay={100} />
          <KpiCard label={t('dashboard.admin.upcomingCourses')} value={kpis.upcomingCourses ?? 0} icon={KpiIcons.upcoming} delay={125} />
          <KpiCard label={t('dashboard.admin.totalLessons')} value={kpis.totalLessons ?? 0} icon={KpiIcons.lessons} delay={150} />
          <KpiCard label={t('dashboard.admin.unpaidPayments')} value={kpis.unpaidCount ?? 0} icon={KpiIcons.unpaid} delay={200} />
          <KpiCard label={t('dashboard.admin.paymentsDueSoon')} value={kpis.dueSoonCount ?? 0} icon={KpiIcons.dueSoon} delay={250} />
        </div>
      </div>
      ...
      >
        {!hasEnoughData ? (
          <EmptyChart message={t('dashboard.admin.notEnoughData', 'Pas encore assez de données')} />
        ) : (
          <ResponsiveContainer width="100%" height={CHART_HEIGHT} debounce={50}>
            <LineChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={gridStroke()} vertical={false} />
              <XAxis dataKey="monthLabel" tick={axisStyle().tick} axisLine={axisStyle().axisLine} tickLine={axisStyle().tickLine} />
              <YAxis tick={axisStyle().tick} axisLine={axisStyle().axisLine} tickLine={axisStyle().tickLine} allowDecimals={false} domain={[0, (dataMax) => Math.max(dataMax || 0, 1)]} />
              ...
              />
              <Line type="monotone" dataKey="new" stroke={CHART_COLORS.new} strokeWidth={2}
                dot={{ fill: CHART_COLORS.new, r: 4 }} activeDot={{ r: 6 }} name="new"
                isAnimationActive animationDuration={800} animationEasing="ease-out" />
              <Line type="monotone" dataKey="cancelled" stroke={CHART_COLORS.cancelled} strokeWidth={2}
                dot={{ fill: CHART_COLORS.cancelled, r: 4 }} activeDot={{ r: 6 }} name="cancelled"
                isAnimationActive animationDuration={800} animationEasing="ease-out" />
              <Line type="monotone" dataKey="active" stroke={CHART_COLORS.active} strokeWidth={2}
                dot={{ fill: CHART_COLORS.active, r: 4 }} activeDot={{ r: 6 }} name="active"
                isAnimationActive animationDuration={800} animationEasing="ease-out" />
            </LineChart>
          </ResponsiveContainer>
        )}
            ...
            {!hasEnoughData ? (
              ...
              <ResponsiveContainer width="100%" height={CHART_HEIGHT} debounce={50}>
                <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke={gridStroke()} vertical={false} />
                  <XAxis dataKey="monthLabel" tick={axisStyle().tick} axisLine={axisStyle().axisLine} tickLine={axisStyle().tickLine} />
                  <YAxis tick={axisStyle().tick} axisLine={axisStyle().axisLine} tickLine={axisStyle().tickLine} domain={[0, (dataMax) => Math.max(dataMax || 0, 1)]} />
                  <Tooltip contentStyle={tooltipStyle()} formatter={(value) => [`€${Number(value).toFixed(0)}`, '']} />
                  <Area type="monotone" dataKey="revenue" stroke={CHART_COLORS.revenue} strokeWidth={2}
