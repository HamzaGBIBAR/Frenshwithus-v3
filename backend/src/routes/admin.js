...
import { buildStudentScheduleMatches } from '../lib/scheduleMatcher.js';
...
// Smart student ↔ teacher schedule (Morocco week grid) with green/orange/red matches.
router.get('/matches/student-schedule', async (req, res) => {
  const studentId = req.query.studentId;
  ...
  try {
    const result = await buildStudentScheduleMatches({ prisma, studentId, weekStart, durationMin });
    if (result?.error) return res.status(404).json({ error: result.error });
    res.json(result);
  } catch (err) {
    console.error('[student-schedule]', err);
    res.status(500).json({ error: err.message || 'Failed to build student schedule' });
  }
});
...
// Teacher (professor) statistics for admin dashboard — rich data for Professional Statistics section
  ...
  try {
    ...
    const professors = await prisma.user.findMany({
      ...
      select: {
        ...
        coursesAsProfessor: {
          select: { id: true, date: true, time: true, startUtc: true, absenceReason: true, isStarted: true, studentId: true },
        },
      },
    });
    ...
    const professorsData = professors.map((p) => {
      const hasAvailability = p.availability && p.availability.length > 0;
      ...
      for (const c of p.coursesAsProfessor) {
        uniqueStudents.add(c.studentId);
        if (endedCourseIds.has(c.id)) completedLessons++;
        ...
        if (c.date >= weekStart) professorIdsActiveThisWeek.add(p.id);
        if (c.date >= today) upcomingCount++;
      }
      ...
      return {
        ...
        absentCount,
        hasAvailability,
        activeThisWeek: professorIdsActiveThisWeek.has(p.id),
      };
    });

    const withAvailabilityCount = professors.filter((p) => p.availability && p.availability.length > 0).length;
    const topByLessons = [...professorsData]
      .filter((p) => p.completedLessons > 0)
      .sort((a, b) => b.completedLessons - a.completedLessons)
      .slice(0, 5);
    const topByUpcoming = [...professorsData]
      .filter((p) => p.upcomingCount > 0)
      .sort((a, b) => b.upcomingCount - a.upcomingCount)
      .slice(0, 5);
    const needsAttention = professorsData.filter((p) => !p.hasAvailability || p.absentCount > 0);
    ...
    captureException(err);
    res.status(500).json({ error: err?.message || 'Teacher analytics failed' });
  }
});
